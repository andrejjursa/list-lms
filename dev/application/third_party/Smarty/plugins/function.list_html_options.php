 ...
 */
function smarty_function_list_html_options($params, $template)
{
    ...
    if (isset($options)) {
        foreach ($options as $_key => $_val) {
            $_html_result .= smarty_function_list_html_options_optoutput($_key, $_val, $selected, $id, $class, $_idx);
        }
    } else {
...
function smarty_function_list_html_options_optoutput($key, $value, $selected, $id, $class, &$idx)
{
    $CI =& get_instance();
    if (!is_array($value)) {
        ...
        if (is_object($value)) {
            if (method_exists($value, "__toString")) {
                $value = smarty_function_escape_special_chars($CI->lang->text((string) $value->__toString()));
            } else {
                trigger_error("html_options: value is an object of class '". get_class($value) ."' without __toString() method", E_USER_NOTICE);
                return '';
            }
        } else {
            $value = smarty_function_escape_special_chars($CI->lang->text((string) $value));
        }
        $_html_result .= $_html_class . $_html_id . '>' . $value . '</option>' . "\n";
        $idx++;
    } else {
        $_idx = 0;
        $_html_result = smarty_function_list_html_options_optgroup($key, $value, $selected, !empty($id) ? ($id.'-'.$idx) : null, $class, $_idx);
        $idx++;
    }
    return $_html_result;
} 
