...
class Plupload {
     ...
     * @return boolean TRUE, when directory is scaned and files are removed, FALSE when target directory is not directory or can't be opened.
     */
    private function remove_temporary_files($targetDir, $filePath = '') {
        ...
        if (is_dir($targetDir) && ($dir = opendir($targetDir))) {
            while (($file = readdir($dir)) !== false) {
                ...
                if (preg_match('/\.upload_part$/', $file) && (filemtime($tmpfilePath) < time() - $maxFileAge) && ($tmpfilePath != "{$filePath}.upload_part")) {
                    @unlink($tmpfilePath);
                }
            }
            closedir($dir);
