{if $course->exists()}
    ...
    <table id="valutation_table">
        <thead>
            ...
            <tr>
                <th rowspan="2">{translate line='admin_solutions_valuation_tables_table_header_student'}</th>
                {if count($table_data.header.content_type_task_set.items) gt 0}
                <th class="content_type_task_sets" colspan="{$table_data.header.content_type_task_set.items|count + 1}">{$table_data.header.content_type_task_set.content_type_name}</th>
                {/if}
                {if count($table_data.header.content_type_project.items) gt 0}
                <th class="content_type_projects" colspan="{$table_data.header.content_type_project.items|count + 1}">{$table_data.header.content_type_project.content_type_name}</th>
                {/if}
                <th rowspan="2">{translate line='admin_solutions_valuation_tables_table_header_total'}</th>
            </tr>
            <tr>
                {foreach $table_data.header.content_type_task_set.items as $header_item}
                    {if $header_item.type eq 'task_set_type'}
                        <th class="type_{$header_item.type}" title="{$header_item.title|escape:'html'}">{$header_item.name|str_to_first_upper}</th>    
                    {else}
                        <th class="type_{$header_item.type}" title="{$header_item.name|escape:'html'}{if $header_item.title} | {$header_item.title|escape:'html'}{/if}">{$header_item.name|abbreviation}</th>
                    {/if}
                {/foreach}
                {if count($table_data.header.content_type_task_set.items) gt 0}
                    <th>{translate line='admin_solutions_valuation_tables_table_header_task_sets_subtotal'}</th>
                {/if}
                {foreach $table_data.header.content_type_project.items as $header_item}
                    <th class="type_{$header_item.type}" title="{$header_item.name|escape:'html'}">{$header_item.name|abbreviation}</th>
                {/foreach}
                {if count($table_data.header.content_type_project.items) gt 0}
                    <th>{translate line='admin_solutions_valuation_tables_table_header_projects_subtotal'}</th>
                {/if}
            </tr>
            {else}
            <tr>
                <th>{translate line='admin_solutions_valuation_tables_table_header_student'}</th>
                <th>{translate line='admin_solutions_valuation_tables_table_header_total'}</th>
            </tr>
            {/if}
        </thead>
        <tbody>
            ...
            <tr class="gradeA">
                <td><a href="mailto:{$row.email}">{$row.fullname}</a></td>
                {if count($row.task_sets_points) gt 0}
                    {foreach $row.task_sets_points as $item}
                        <td class="type_{$item.type} flag_{$item.flag}" title="{translate line="admin_solutions_valuation_tables_table_body_flag_{$item.flag}"}">{$item.points}</td>
                    {/foreach}
                    <td class="type_summary">{$row.task_sets_points_total}</td>
                {/if}
                {if count($row.projects_points) gt 0}
                    {foreach $row.projects_points as $item}
                        <td class="type_{$item.type} flag_{$item.flag}" title="{translate line="admin_solutions_valuation_tables_table_body_flag_{$item.flag}"}">{$item.points}</td>
                    {/foreach}
                    <td class="type_summary">{$row.projects_points_total}</td>
                {/if}
