            ...
            <div class="groups_wrap">
                ...
                {foreach $course->group->order_by_with_constant('name', 'asc')->get_iterated() as $group}
                    ...
                    <div class="group_wrap">
                        ...
                        {foreach $group->participant->include_related('student', 'fullname')->where_related($course)->where('allowed', 1)->order_by_related('student', 'fullname', 'asc')->get_iterated() as $participant}
                            <li>{$participant->student_fullname}</li>
                        {foreachelse}
                ...
                <div class="group_wrap">
                    ...
                    <ul class="group_students">
                    {foreach $course->participant->include_related('student', 'fullname')->where('allowed', 1)->where('group_id', null)->order_by_related('student', 'fullname', 'asc')->get_iterated() as $participant}
                        <li>{$participant->student_fullname}</li>
                    {foreachelse}
                            <li>{translate line='groups_group_is_empty'}</li>
                    {/foreach}
