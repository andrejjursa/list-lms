...
<ul{if $level eq 0} id="jMenu"{/if}>
    ...
    <li class="{if $current eq $item.pagetag || $subitemselected|trim eq 'yes'}selected{/if}"><a href="{if $item.link|substr:0:9|upper eq 'EXTERNAL:'}{$item.link|substr:9}{else}{internal_url url=$item.link}{/if}" class="{if $level eq 0} fNiv{/if}">{translate_text text=$item.title}</a>{make_adminmenu menu=$item.sub current=$current level=$level+1}</li>
    {/foreach}
</ul>
{/if}
{/function}