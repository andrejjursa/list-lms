...
<ul>
    ...
    <li class="{if $current eq $item.pagetag || $subitemselected|trim eq 'yes'}selected{/if}"><a href="{if $item.link|substr:0:9|upper eq 'EXTERNAL:'}{$item.link|substr:9}{else}{internal_url url=$item.link}{/if}">{translate_text text=$item.title}</a>{make_adminmenu menu=$item.sub current=$current}</li>
    {/foreach}
</ul>
{/if}
{/function}