...
<div class="field">
    ...
    <p class="input"><select name="solution[student_id]" size="1" id="solution_student_id_id">{list_html_options options=$students selected=$smarty.post.solution.student_id|intval}</select></p>
    {form_error field='solution[student_id]' left_delimiter='<p class="error"><span class="message">' right_delimiter='</span></p>'}
</div>
<div class="field">
    <label for="solution_points_id">{translate line='admin_solutions_list_form_label_points'}:</label>
...
<div class="buttons">
    ...
    {if $last_created_solution_id}
    <input type="hidden" name="last_created_solution_id" value="{$last_created_solution_id|intval}" />
    {/if}
</div>