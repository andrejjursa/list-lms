...
class DMZ_Ordering
{
    ...
    {
        $object->db->ar_orderby[] = 'SUBSTRING_INDEX(`' . $column . '`, \' \', -1) '
    ...
    public function order_by_related_as_fullname($object, $related, $column, $direction = 'asc')
    {
        ...
        $object->db->ar_orderby[] = 'SUBSTRING_INDEX(' . $object->db->protect_identifiers($related_table)
            . '.`' . $column . '`, \' \', -1) ' . (strtolower($direction) === 'asc' ? 'ASC' : 'DESC');
