...
class DMZ_RowIndex
{
     ...
     * @param array          $leave_select A list of items to leave in the selection array, overriding the automatic
     *                                     removal.
     ...
     * @param array                $leave_select A list of items to leave in the selection array, overriding the
     *                                           automatic removal.
    ...
    public function row_indices($object, $ids, $leave_select = [], $distinct_on = false)
    {
        ...
        if ($distinct_on) {
            ...
            $distinct = 'DISTINCT ON (' . preg_replace(
                    "/\s+(asc|desc)/i",
                    "",
                    implode(",", $object->db->ar_orderby)
                ) . ') ';
