...
class DMZ_Nestedsets
{
     ...
     * @access public
     */
    function tree_config($object, $options = [])
    {
    ...
    function is_valid_node($object)
    {
        ...
        if (!isset($object->{$this->_leftindex}) or !is_numeric($object->{$this->_leftindex})
            or $object->{$this->_leftindex} <= 0
        ) {
            return false;
        }
    
        if (!isset($object->{$this->_rightindex}) or !is_numeric($object->{$this->_rightindex})
            or $object->{$this->_rightindex} <= 0
        ) {
            return false;
        }
    
        if ($object->{$this->_leftindex} >= $object->{$this->_rightindex}) {
            return false;
        }
    
        if (!empty($this->_rootfield) && !in_array($this->_rootfield, $object->fields)) {
            return false;
        }
    
        if (!empty($this->_rootfield) && (!is_numeric($object->{$this->_rootfield})
            or $object->{$this->_rootfield} <= 0)
        ) {
            return false;
        }
    ...
    function remove_tree($object, $tree_id = null)
    {
        ...
        if (in_array($this->_rootfield, $object->fields)) {
            ...
            } else if (!is_null($this->_rootindex)) {
                // only delete the selected one
