...
class Tasks extends LIST_Controller
{
    ...
    public function index(): void
    {
        ...
        if (is_array($data) && array_key_exists('show_all_task_sets', $data)
            && (int)$data['show_all_task_sets'] === 1
        ) {
        ...
        if (!$this->_is_cache_enabled()
            || !$this->parser->isCached($this->parser->find_view('frontend/tasks/index.tpl'), $cache_id)
        ) {
    ...
    public function task($task_set_id_url = null): void
    {
        ...
        if (!$this->_is_cache_enabled()
            || !$this->parser->isCached($this->parser->find_view('frontend/tasks/task.tpl'), $cache_id)
        ) {
            ...
            if ($course->exists()) {
                ...
                if ($filtered_task_set->exists()) {
                    ...
                    $this->parser->assign(
                        'task_set_can_upload',
                        $this->can_upload_file($filtered_task_set, $course)
                    );
                    ...
                    $this->messages->add_message(
                        'lang:tasks_task_task_set_not_found',
        ...
        if ($test_queue->exists()) {
            ...
            foreach ($tests as $test) {
                $test_line = [
                    ...
                    'evaluation_table' => is_null($test->join_evaluation_table)
                        ? []
                        : unserialize($test->join_evaluation_table),
                ];
    ...
    public function upload_solution($task_set_id = 0): void
    {
        ...
        if ($filtered_task_set->id === (int)$task_set_id && $this->can_upload_file($filtered_task_set, $course)) {
            $allowed_file_types_array = trim($filtered_task_set->allowed_file_types) !== ''
            ...
            if ($this->upload->do_upload('file')) {
                ...
                if ((is_string($mimes) && $upload_data['file_type'] !== $mimes)
                    || (is_array($mimes) && !in_array($upload_data['file_type'], $mimes, true))
                ) {
                    if (!$this->zip_plain_file_to_archive(
                        ...
                        $this->messages->add_message(
                            'lang:tasks_task_error_cant_zip_file',
                            Messages::MESSAGE_TYPE_ERROR
                        );
                ...
                $solution->get();
                $revalidate = 1;
                if ($filtered_task_set->enable_tests_scoring == 1 && $filtered_task_set->allowed_test_types !== ''
                    && $course->test_scoring_deadline >= date('Y-m-d H:i:s')
                ) {
                ...
                if ($this->db->trans_status()) {
                    ...
                    $log->add_student_solution_upload_log(
                        sprintf(
                            $this->lang->line('tasks_task_solution_upload_log_message'),
                            $config['file_name']
                        ),
                        $student,
                        $solution->id
                    ...
                    $this->messages->add_message(
                        'lang:tasks_task_solution_uploaded',
                    ...
                    $this->messages->add_message(
                        'lang:tasks_task_solution_canceled_due_db_error',
                        Messages::MESSAGE_TYPE_ERROR
                    );
                }
                redirect(create_internal_url('tasks/task/' . (int)$task_set_id));
            ...
            $this->messages->add_message(
                'lang:tasks_task_error_cant_upload_solution',
                Messages::MESSAGE_TYPE_ERROR
            );
            redirect(create_internal_url('tasks/task/' . (int)$task_set_id));
        }
            ...
            if ($task_set->exists()) {
                ...
                if ($file_info !== false) {
                    ...
                    if ($allow_download) {
                        $log = new Log();
                        if (!$this->usermanager->is_teacher_session_valid()) {
                            $log->add_student_solution_download_log(
                                $this->lang->line('tasks_log_message_student_solution_download'),
                                $this->usermanager->get_student_id(),
                                $filename,
                                $task_set->id
                            );
                    ...
                    $this->parser->parse(
                        'frontend/tasks/download_solution.tpl',
                        [
                            'version_download_disabled' => true,
                        ]
                    );
                } else {
    ...
    public function show_comments($task_id): void
    {
        ...
        $this->parser->parse(
            'frontend/tasks/show_comments.tpl',
            [
                'comments' => $comments,
                'task_set' => $task_set,
            ]
    ...
    public function subscribe_to_task_comments($task_id): void
    {
        ...
        if ($task_set->exists() && $student->exists() && $student->save(['comment_subscription' => $task_set])) {
            $this->db->trans_commit();
            $this->messages->add_message(
                'lang:tasks_comments_message_subscription_successful',
                Messages::MESSAGE_TYPE_SUCCESS
            );
        } else {
            $this->db->trans_rollback();
            $this->messages->add_message(
                'lang:tasks_comments_message_subscription_error',
                Messages::MESSAGE_TYPE_ERROR
            );
        }
    ...
    public function unsubscribe_to_task_comments($task_id): void
    {
        ...
        if ($task_set->exists() && $student->exists()
            && $student->is_related_to('comment_subscription', $task_set->id)
        ) {
            $student->delete_comment_subscription($task_set);
            if ($this->db->trans_status()) {
                $this->db->trans_commit();
                $this->messages->add_message(
                    'lang:tasks_comments_message_subscription_cancel_successful',
                    Messages::MESSAGE_TYPE_SUCCESS
                );
            } else {
                $this->db->trans_rollback();
                $this->messages->add_message(
                    'lang:tasks_comments_message_subscription_cancel_error',
                    Messages::MESSAGE_TYPE_ERROR
                );
            }
        } else {
    ...
    public function reply_at_comment($task_id, $comment_id): void
    {
        ...
        $this->parser->add_css_file('frontend_tasks.css');
        $this->parser->parse(
            'frontend/tasks/reply_at_comment.tpl',
            [
                'task_set' => $task_set,
                'comment'  => $comment,
            ]
    ...
    public function save_solution_version_comment($id): void
    {
        ...
        if ($solution_version->exists()) {
            ...
            $this->db->trans_commit();
            $this->messages->add_message(
                'lang:tasks_solution_version_comment_updated',
                Messages::MESSAGE_TYPE_SUCCESS
            );
        } else {
            $this->db->trans_rollback();
            $this->messages->add_message(
                'lang:tasks_solution_version_not_found',
                Messages::MESSAGE_TYPE_ERROR
            );
        }
    ...
    private function create_comment(): bool
    {
        ...
        if (array_key_exists('text', $post_data) && array_key_exists('task_set_id', $post_data)
            && array_key_exists('reply_at_id', $post_data)
        ) {
            $task_set = new Task_set();
            ...
            if ($task_set->exists() && $student->exists() && (bool)$task_set->comments_enabled) {
                if (trim(strip_tags($post_data['text'])) !== '') {
                    ...
                    if ($comment->save([$task_set, $student])) {
                        $this->db->trans_commit();
                        $this->messages->add_message(
                            'lang:tasks_comments_message_comment_post_success_save',
                            Messages::MESSAGE_TYPE_SUCCESS
                        );
                        if ((bool)$comment->approved) {
                            ...
                            $this->_send_multiple_emails(
                                ...
                                ['task_set' => $task_set, 'student' => $student, 'comment' => $comment]
                            );
                            $task_set_related_teachers = new Teacher();
                            if (!is_null($task_set->group_id)) {
                                $task_set_related_teachers->where_related(
                                    'room/group',
                                    'id',
                                    $task_set->group_id
                                );
                    ...
                    $this->db->trans_rollback();
                    $this->messages->add_message(
                        'lang:tasks_comments_message_comment_post_error_save',
                        Messages::MESSAGE_TYPE_ERROR
                    );
                    return false;
                }
                ...
                $this->messages->add_message(
                    'lang:tasks_comments_message_comment_post_error_empty',
                    Messages::MESSAGE_TYPE_ERROR
                );
                return false;
            }
            ...
            $this->messages->add_message(
                'lang:tasks_comments_message_not_found_or_disabled',
                Messages::MESSAGE_TYPE_ERROR
            );
            return false;
        }
        ...
        $this->messages->add_message(
            'lang:tasks_comments_message_comment_post_error_data',
            Messages::MESSAGE_TYPE_ERROR
        );
        return false;
    }
    ...
    private function get_task_sets(&$course, &$group, &$student): Task_set
    {
        ...
        if ($course->exists()) {
            ...
            $task_set->select(
                ...
                . '`task_sets`.`upload_end_time` AS `pb_upload_end_time`'
            );
            ...
            $task_set->select_subquery(
                '(SELECT `upload_solution` FROM `course_task_set_type_rel` '
                . 'WHERE `course_task_set_type_rel`.`course_id` = `${parent}`.`course_id` '
                . 'AND `course_task_set_type_rel`.`task_set_type_id` = `${parent}`.`task_set_type_id`)',
                'join_upload_solution'
            );
            $task_set->where('content_type', 'task_set');
            
            $task_set2->select(
                '`task_sets`.*, `task_set_permission_rooms`.`time_day` AS `pb_time_day`, '
                . '`task_set_permission_rooms`.`time_begin` AS `pb_time_begin`, '
                . '`task_set_permission_rooms`.`id` AS `pb_room_id`, '
                . '`task_set_permissions`.`publish_start_time` AS `pb_publish_start_time`, '
                . '`task_set_permissions`.`upload_end_time` AS `pb_upload_end_time`'
            );
            ...
            $task_set2->include_related(
                'task_set_permission/room',
                '*',
                'room',
                true
            );
            ...
            $task_set3->select(
                '`task_sets`.*, NULL AS `pb_time_day`, NULL AS `pb_time_begin`, NULL AS `pb_room_id`, '
                . 'NULL AS `pb_publish_start_time`, "0000-00-00 00:00:00" AS `pb_upload_end_time`',
                false
            );
            ...
            $sorting = $task_set2->union_order_by_overlay(
                'task_set_type_name',
                'task_set_types',
                'name',
                'task_set_type_id',
                        ...
                        if (strtotime($task_set->pb_publish_start_time) > time()) {
                            ...
                            if ($task_set->pb_time_day === $current_day) {
                                [$year, $month, $day] = explode(
                                    ',',
                                    strftime('%Y,%m,%d', strtotime($task_set->pb_publish_start_time))
                                );
                                $time = mktime(
                                        0,
                                        0,
                                        ...
                                        (int)$day,
                                        (int)$year
                                    ) + (int)$task_set->pb_time_begin;
                            } else {
                                $time = strtotime(
                                        'next ' . $days[$task_set->pb_time_day],
                                        strtotime($task_set->pb_publish_start_time)
                                    ) + (int)$task_set->pb_time_begin;
                            }
                            if ($time > time()) {
                                $add = false;
    ...
    private function compute_points($i_task_sets, Student $student): array
    {
        ...
        if (count($task_sets) > 0) {
            foreach ($task_sets as $task_set) {
                ...
                $output[$task_set->task_set_type_id]['max'] = ($output[$task_set->task_set_type_id]['max'] ?? 0) + (
                    !is_null($task_set->points_override)
                        ? $task_set->points_override
                        : $task_set->total_points
                    );
            }
