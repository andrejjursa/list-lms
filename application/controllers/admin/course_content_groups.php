...
class Course_content_groups extends LIST_Controller
{
    ...
    public const STORED_FILTER_SESSION_NAME = 'admin_course_content_groups_filter_data';
    ...
    public function get_all_content_groups(): void
    {
        ...
        if (isset($filter['course_id']) && (int)$filter['course_id'] > 0) {
            $content_groups->where_related_course('id', (int)$filter['course_id']);
        }
        ...
        } else if ($filter['order_by_field'] === 'content_count') {
    ...
    private function inject_courses_for_edit(Course_content_group $content_group): void
    {
        if ($content_group->exists()) {
            $course_content = new Course_content_model();
            $course_content->where_related($content_group);
