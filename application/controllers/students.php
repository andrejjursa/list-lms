...
class Students extends LIST_Controller
{
        ...
        if ($this->form_validation->run()) {
            ...
            if ($student->exists()) {
                ...
                if ($student->save()) {
                    ...
                    $this->email->build_message_body('file:emails/frontend/students/password_recovery.tpl', ['student' => $student]);
    ...
    public function change_password($token, $encoded_email): void
    {
        ...
        if ($this->form_validation->valid_email($email) && preg_match('/^[0-9a-f]{40}$/', $token)) {
            ...
            if ($student->exists()) {
                $this->_init_language_for_student($student);
                $this->parser->parse('frontend/students/change_password.tpl', ['student' => $student, 'token' => $token, 'encoded_email' => $encoded_email]);
            } else {
    ...
    public function my_account(): void
    {
        ...
        $this->parser->parse('frontend/students/my_account.tpl', [], false, $this->_is_cache_enabled(), $cache_id);
    }
