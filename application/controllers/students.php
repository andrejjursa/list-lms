...
class Students extends LIST_Controller
{
    ...
    private $student_registration_config;
    ...
    public function login(): void
    {
        ...
        $this->parser->parse('frontend/students/login.tpl', ['uri_params' => $uri_params]);
    }
    ...
    public function do_login(): void
    {
        ...
        $this->form_validation->set_rules(
            'student[email]',
            'lang:students_login_field_email',
            'required|valid_email'
        );
        $this->form_validation->set_rules(
            'student[password]',
            'lang:students_login_field_password',
        ...
        if ($this->form_validation->run()) {
            ...
            if ($this->usermanager->is_login_attempts_exceeded(
                $student_data['email'],
                Usermanager::ACCOUNT_TYPE_STUDENT
            )) {
                $message = sprintf(
                    $this->lang->line('students_login_error_attempts_exceeded'),
                    $this->config->item('student_login_security_allowed_attempts'),
                    $this->config->item('student_login_security_timeout')
                );
                $this->parser->assign('general_error', $message);
                $this->login();
            } else if ($this->usermanager->authenticate_student_login(
                $student_data['email'],
                $student_data['password']
            )) {
                $this->messages->add_message(
                    'lang:students_login_successful',
                ...
                $this->parser->assign(
                    'general_error',
                    $this->lang->line('students_login_error_bad_email_or_password')
                );
                $this->login();
            }
        } else {
    ...
    public function logout(): void
    {
        ...
        $this->messages->add_message(
            'lang:students_logout_logout_message',
            Messages::MESSAGE_TYPE_SUCCESS
        );
    ...
    public function logout_to_page($course_id, $lang = null): void
    {
        $this->usermanager->do_student_logout();
        $this->messages->add_message(
            'lang:students_logout_logout_message',
     ...
     */
    public function password_recovery(): void
    {
        $uri_params = $this->uri->uri_to_assoc(3);
        $this->parser->parse('frontend/students/password_recovery.tpl', ['uri_params' => $uri_params]);
    }
    
    /**
     * Sends password recovery e-mail to student.
     */
    public function do_password_recovery(): void
    {
        $uri_params = $this->uri->uri_to_assoc(3);
        ...
        $this->form_validation->set_rules(
            'student[email]',
            'lang:student_password_recovery_form_field_email',
            'required|valid_email'
        );
            ...
            if ($student->exists()) {
                ...
                if ($student->save()) {
                    ...
                    $this->email->build_message_body(
                        'file:emails/frontend/students/password_recovery.tpl',
                        ['student' => $student]
                    );
                    $this->email->subject(
                        'LIST - ' . $this->lang->line('students_password_recovery_email_body_subject')
                    );
                    if ($this->email->send()) {
                        $this->messages->add_message(
                            'lang:students_password_recovery_email_sent',
                            Messages::MESSAGE_TYPE_SUCCESS
                        );
                    } else {
                        $this->messages->add_message(
                            'lang:students_password_recovery_email_sent_error',
     ...
     */
    public function change_password($token, $encoded_email): void
    {
        if ($this->usermanager->is_student_session_valid()) {
            $this->messages->add_message(
                'lang:students_change_password_student_loged_in',
                Messages::MESSAGE_TYPE_ERROR
            );
            redirect('/');
        }
        ...
        if ($this->form_validation->valid_email($email) && preg_match('/^[0-9a-f]{40}$/', $token)) {
            ...
            if ($student->exists()) {
                $this->_init_language_for_student($student);
                $this->parser->parse(
                    'frontend/students/change_password.tpl',
                    [
                        'student'       => $student,
                        'token'         => $token,
                ...
                $this->messages->add_message(
                    'lang:students_change_password_invalid_token_email',
                    Messages::MESSAGE_TYPE_ERROR
                );
    ...
    public function do_change_password($token, $encoded_email): void
    {
        if ($this->usermanager->is_student_session_valid()) {
            $this->messages->add_message(
                'lang:students_change_password_student_loged_in',
        ...
        if ($this->form_validation->valid_email($email) && preg_match('/^[0-9a-f]{40}$/', $token)) {
            ...
            $this->form_validation->set_rules(
                'student[password]',
                'lang:students_change_password_form_field_password',
                'required|min_length[6]|max_length[20]'
            );
            $this->form_validation->set_rules(
                'student[verify]',
                'lang:students_change_password_form_field_verify',
                'required|matches[student[password]]'
            );
            if ($this->form_validation->run()) {
                $student_post = $this->input->post('student');
                ...
                if ($student->save()) {
                    $this->db->trans_commit();
                    $this->messages->add_message(
                        'lang:students_change_password_success',
                        Messages::MESSAGE_TYPE_SUCCESS
                    );
                    redirect(create_internal_url('students/login'));
                } else {
                    $this->db->trans_rollback();
                    $this->messages->add_message(
                        'lang:students_change_password_failed',
                        Messages::MESSAGE_TYPE_ERROR
                    );
                    redirect(create_internal_url('students/login'));
                }
    ...
    public function registration(): void
    {
        if ($this->student_registration_config['enabled']) {
            ...
            $this->messages->add_message(
                'lang:students_registration_disabled',
                Messages::MESSAGE_TYPE_ERROR
            );
            redirect('/');
        }
    ...
    public function do_registration(): void
    {
        if ($this->student_registration_config['enabled']) {
            ...
            $this->form_validation->set_rules(
                'student[fullname]',
                'lang:students_registration_validation_field_fullname',
                'required'
            );
            $this->form_validation->set_rules(
                'student[email]',
                'lang:students_registration_validation_field_email',
                'required|valid_email|is_unique[students.email]'
            );
            $this->form_validation->set_rules(
                'student[password]',
                'lang:students_registration_validation_field_password',
                'required|min_length[6]|max_length[20]'
            );
            $this->form_validation->set_rules(
                'student[password_verification]',
                'lang:students_registration_validation_field_password_verification',
                'required|matches[student[password]]'
            );
            if ($this->form_validation->run()) {
                ...
                $student = new Student();
                $student->from_array($student_array, ['fullname', 'email', 'password']);
    ...
    public function my_account(): void
    {
        ...
        if (!$this->_is_cache_enabled()
            || !$this->parser->isCached($this->parser->find_view('frontend/students/my_account.tpl'), $cache_id)
        ) {
            $student = new Student();
            $student->get_by_id($this->usermanager->get_student_id());
            
            $languages_available = $this->lang->get_list_of_languages();
            $this->parser->assign(['student' => $student, 'languages' => $languages_available]);
        }
        $this->parser->parse(
            'frontend/students/my_account.tpl',
            [],
            false,
    ...
    public function save_basic_information(): void
    {
        ...
        $this->form_validation->set_rules(
            'student[fullname]',
            'lang:students_my_account_field_fullname',
            'required|max_length[255]'
        );
        $this->form_validation->set_rules(
            'student[language]',
            'lang:students_my_account_field_language',
            'required'
        );
        ...
        if ($this->form_validation->run()) {
            ...
            if ($student_id === $this->usermanager->get_student_id()) {
                ...
                if ($student->exists()) {
                    $student->from_array($this->input->post('student'), ['fullname', 'language']);
                    if ($student->save() && $this->db->trans_status()) {
                        $this->db->trans_commit();
                        $this->messages->add_message(
                            'lang:students_my_account_success_save',
                            Messages::MESSAGE_TYPE_SUCCESS
                        );
                        ...
                        $this->db->trans_rollback();
                        $this->messages->add_message(
                            'lang:students_my_account_error_save',
                            Messages::MESSAGE_TYPE_ERROR
                        );
                    }
                } else {
                    $this->db->trans_rollback();
                    $this->messages->add_message(
                        'lang:students_my_account_error_invalid_account',
                        Messages::MESSAGE_TYPE_ERROR
                    );
                }
            } else {
    ...
    public function save_password(): void
    {
        ...
        $student_id = (int)$this->input->post('student_id');
        $this->form_validation->set_rules(
            'student[password_old]',
            'lang:students_my_account_field_old_password',
            'required|callback__validate_old_password[' . $student_id . ']'
        );
        $this->form_validation->set_rules(
            'student[password]',
            'lang:students_my_account_field_password',
            'required|min_length[6]|max_length[20]'
        );
        $this->form_validation->set_rules(
            'student[password_validation]',
            'lang:students_my_account_field_password_validation',
            'required|matches[student[password]]'
        );
        $this->form_validation->set_rules('student_id', 'id', 'required');
        $this->form_validation->set_message(
            '_validate_old_password',
            $this->lang->line('students_my_account_field_old_password_error_message')
        );
    ...
    public function _validate_old_password($str, $student_id): bool
    {
        $student = new Student();
    ...
    public function save_email(): void
    {
        ...
        if ((bool)$this->config->item('student_mail_change')) {
            ...
            $this->form_validation->set_rules(
                'student[email]',
                'lang:students_my_account_field_email',
                'required|valid_email|is_unique[students.email]'
            );
            $this->form_validation->set_rules(
                'student[email_validation]',
                'lang:students_my_account_field_email_validation',
                'required|matches[student[email]]'
            );
            $this->form_validation->set_rules('student_id', 'id', 'required');
    ...
    public function crop_avatar(): void
    {
        $this->usermanager->student_login_protected_redirect();
    ...
    public function save_avatar(): void
    {
        $this->usermanager->student_login_protected_redirect();
        ...
        if (file_exists($full_path)) {
            if (is_array($crop) && array_key_exists('x', $crop) && array_key_exists('y', $crop)
                && array_key_exists('width', $crop) && array_key_exists('height', $crop)
                && (int)$crop['x'] >= 0 && (int)$crop['y'] >= 0 && (int)$crop['width'] > 0 && (int)$crop['height'] > 0
            ) {
                @unlink($big_path);
                @unlink($avatar_path);
                ...
                $config['x_axis'] = (int)$crop['x'];
                $config['y_axis'] = (int)$crop['y'];
                ...
                if ($this->image_lib->crop()) {
                    $this->messages->add_message(
                        'lang:students_upload_avatar_messaeg_save_successful',
                        Messages::MESSAGE_TYPE_SUCCESS
                    );
                    $this->_action_success();
                } else {
                    $this->messages->add_message(
                        'lang:students_upload_avatar_messaeg_save_failed',
                        Messages::MESSAGE_TYPE_ERROR
                    );
                }
            } else {
                $this->messages->add_message(
                    'lang:students_upload_avatar_messaeg_invalid_crop_params',
                    Messages::MESSAGE_TYPE_ERROR
                );
            }
            @unlink($full_path);
            ...
            $this->messages->add_message(
                'lang:students_upload_avatar_messaeg_file_not_found',
                Messages::MESSAGE_TYPE_ERROR
            );
        }
