 ...
 * @author  Andrej Jursa
 */
class asm_test extends abstract_test
{
    ...
    protected $test_subtypes = [
        'simple_test' => [
            'name'                  => 'lang:asm_tests_subtype_simple_test_name',
            'method'                => 'run_simple_test',
            'configure_view'        => 'tests/asm/configure.tpl',
    ...
    {
        return $this->CI->lang->line('tests_asm_type_name');
    }
    ...
    protected function run_simple_test($save_score = false, $score_token = '', $score_student = null): string
    {
        ...
        if (!preg_match(self::UNIT_TEST_CLASS_TO_RUN_REGEXP, $class_to_run)) {
            set_time_limit(120);
            return $this->CI->lang->line('asm_tests_run_error_simple_test_file_not_set');
        }

        $exec_command = $scripts_directory . 'start_asm_simple_test ' 
            . $this->get_test_timeout() . ' ' . rtrim(getcwd(), '\\/')
            . DIRECTORY_SEPARATOR . $working_directory . ' ' . $class_to_run;
        $output_data = [];
        $exit_code = 0;
    ...
    protected function validator_simple_test(): bool
    {
        $this->CI->form_validation->set_rules(
            ...
            'lang:asm_tests_config_validation_simple_test_scoring_percents',
