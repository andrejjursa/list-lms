...
function list_import_lamsfet_courses_and_courses_terms(&$courses_terms, $courses)
{
    ...
    if (count($courses_terms)) {
        foreach ($courses_terms as $course_term) {
            $periods[$course_term->year][$course_term->term] = isset($periods[$course_term->year][$course_term->term])
                ? $periods[$course_term->year][$course_term->term]
                : new stdClass();
            $periods[$course_term->year][$course_term->term]->name = (strtoupper($course_term->term) === 'Z'
                    ? 'Zimný semester '
                    : 'Letný semester ') . $course_term->year;
...
function list_import_lamsfet_courses_set_types_relation($courses_set_types, $courses_terms, $set_types)
{
    ...
    if (count($courses_set_types)) {
        foreach ($courses_set_types as $course_set_type) {
            ...
            $task_set_type->get_by_id((int)$task_set_type_id);
            if ($course->exists() && $task_set_type->exists()) {
                $course->save($task_set_type);
                $course->set_join_field(
                    $task_set_type,
                    'upload_solution',
                    $course_set_type->submit_allowed === 't' ? 1 : 0
                );
                echo '.';
            } else {
                echo ' ( TASK SET TYPE OR COURSE NOT FOUND ' . $task_set_type_id . '(' . $course_set_type->set_type_id
                    . ')/' . $course_id . '(' . $course_set_type->course_term_id . ') ) ';
            }
        }
    }
...
function list_import_lamsfet_excercise_groups(&$excercise_groups, $courses_terms)
{
    ...
    $days = [1 => 'Pondelok', 'Utorok', 'Streda', 'Štvrtok', 'Piatok', 'Sobora', 'Nedeľa'];
    ...
    if (count($excercise_groups)) {
        foreach ($excercise_groups as $id => $excercise_group) {
            ...
            if ($course->exists()) {
                $group = new Group();
                [$h, $m, $s] = explode(':', $excercise_group->time);
                $time_begin = (int)$s + (int)$m * 60 + (int)$h * 3600;
                if ($time_begin >= 86400) {
                    $excercise_group->time = '00:00:00';
                    $time_begin = 0;
                }
                $group->name = 'Skupina ' . $excercise_group->place . ' ' . $days[$excercise_group->day]
                    . ' ' . $excercise_group->time;
                $group->save($course);
                ...
                $room->teachers_plain = trim($excercise_group->teacher) !== ''
                    ? trim($excercise_group->teacher)
                    : null;
                $room->save($group);
    ...
    $import_structure = function ($parent_id, $structured_labels, &$labels) use (&$import_structure) {
        if (isset($structured_labels[$parent_id]) && count($structured_labels[$parent_id])) {
            foreach ($structured_labels[$parent_id] as $label) {
                ...
                $category->save(['parent' => $parent_category]);
                echo '.';
...
function list_import_lamsfet_tasks(&$tasks, $lamsfet_url)
{
    ...
    if (count($tasks)) {
        foreach ($tasks as $task_id => $task) {
            ...
            $text_html = str_get_html(
                lamsfet_task_get_formatted_text($task->text),
                true,
                true,
            ...
            foreach ($text_html->find('a') as $element) {
                if (!preg_match('/^[a-zA-Z]+\:\/\//', trim($element->href))) {
                    $element->href = lamsfet_import_task_file_to_local_list_storage(
                        rtrim($lamsfet_url, '\\/') . '/' . trim($element->href),
                        $element->href,
                        $list_task->id,
                        $task->id
                    );
            ...
            foreach ($text_html->find('img') as $element) {
                if (!preg_match('/^[a-zA-Z]+\:\/\//', trim($element->src))) {
                    $element->src = lamsfet_import_task_file_to_local_list_storage(
                        rtrim($lamsfet_url, '\\/') . '/' . trim($element->src),
                        $element->src,
                        $list_task->id,
                        $task->id
                    );
...
function list_import_lamsfet_tasks_labels_relations($tasks, $labels, $tasks_labels)
{
    ...
    if (count($tasks_labels)) {
        foreach ($tasks_labels as $relation) {
            ...
            $category = new Category();
            $category->get_by_id((int)$category_id);
            if ($task->exists() && $category->exists()) {
                $task->save($category);
            } else {
                echo ' ( CATEGORY OR TASK NOT FOUND ' . $task_id . '(' . $relation->task_id . ')/' . $category_id
                    . '(' . $relation->label_id . ') ) ';
            }
            echo '.';
        }
...
function list_import_lamsfet_set_types(&$set_types)
{
    ...
    if (count($set_types)) {
        foreach ($set_types as $id => $set_type) {
            $task_set_type = new Task_set_type();
...
function list_import_lamsfet_sets(&$sets, $set_types, $courses_terms, $excercise_groups)
{
    ...
    if (count($sets)) {
        foreach ($sets as $id => $set) {
            ...
            $task_set->save([$task_set_type, $group, $course]);
    ...
    if (count($tasks_in_sets)) {
        foreach ($tasks_in_sets as $task_in_set) {
            ...
            if ($task_set->exists() && $task->exists()) {
                ...
                $task_set->set_join_field($task, 'points_total', (float)$task_in_set->points);
                $task_set->set_join_field($task, 'bonus_task', 0);
            } else {
                echo ' ( TASK SET OR TASK NOT FOUND ' . $task_set_id . '(' . $task_in_set->set_id . ')/' . $task_id
                    . '(' . $task_in_set->task_id . ') ) ';
            }
            echo '.';
        }
...
function lamsfet_task_get_formatted_text($text)
{
    ...
    foreach ($parts as $part) {
        ...
        $formatted .= "<code>" . htmlspecialchars(str_replace("&#039;", "'", array_shift($code)))
...
function lamsfet_import_task_file_to_local_list_storage($full_path, $original_path, $list_task_id, $lamsfet_task_id)
{
    ...
    echo ' ( FILE NOT FOUND ' . $full_path . ' FOR LaMSfET TASK ' . $lamsfet_task_id
        . ' [LIST ' . $list_task_id . '] ) ';
    log_message('ERROR', 'FILE NOT FOUND ' . $full_path . ' FOR LaMSfET TASK ' . $lamsfet_task_id
        . ' [LIST ' . $list_task_id . ']', false);
    ...
    foreach ($tasks as $task) {
        ...
        foreach ($text_html->find('img, a') as $element) {
            if ($element->tag === 'a' && mb_strpos(trim($element->href), $broken_prefix) === 0) {
                ...
                $save = true;
            } else if ($element->tag === 'img' && mb_strpos(trim($element->src), $broken_prefix) === 0) {
                echo ' BROKEN img TAG' . "\n";
