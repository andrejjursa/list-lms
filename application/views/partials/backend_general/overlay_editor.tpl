...
{foreach $languages as $idiom => $language}
    ...
    <p class="input"><textarea name="overlay[{$idiom}][{$table}][{$table_id}][{$column}]" id="overlay_{$idiom}_{$table}_{$table_id}_{$column}_id"{if $class} class="{$class}"{/if}>{$smarty.post.overlay[$idiom][$table][$table_id][$column]|default:$default_lang_overlay_text|add_base_url}</textarea></p>
    {else}
